...
public class BuggedGrassBlock extends GrassBlock {
    public BuggedGrassBlock(Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            world.setBlockState(pos, ModBlocks.BUGGED_DIRT.getDefaultState());
            return;
        }
        if (world.getLightLevel(pos.up()) >= 9) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (!world.getBlockState(blockPos).isOf(ModBlocks.BUGGED_DIRT) || !canSpread(blockState, world, blockPos)) continue;
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        BlockPos blockPos = pos.up();
        BlockState blockState = ModBlocks.BUGGED_GRASS_BLOCK.getDefaultState();
